...
Run a tool to test tag detection. Options:\n\
 ...
 -i FILEPATH     Set Azure Kinect MKV recording file path\n",
...
CamTestOptions parse_options(int argc, char** argv) {
  CamTestOptions opts;
  const char* options_str = "hDS:s:a:m:V:N:brnf:e:d:F:z:W:H:Mi:";
  int c;
  while ((c = getopt(argc, argv, options_str)) != -1) {
    switch (c) {
      ...
      case 'i': opts.mkv_file_path = optarg; break;
      default:
        fprintf(stderr, "\n");
...
int main(int argc, char** argv) {
  ...
  // Print MKV file path if provided
  if (!opts.mkv_file_path.empty()) {
    std::cout << "\n[MKV Recording Mode]" << std::endl;
    std::cout << "Loading Azure Kinect MKV file: " << opts.mkv_file_path << std::endl;
    std::cout << std::endl;

    // check if file exists 
    std::ifstream mkv_file(opts.mkv_file_path);
    if (!mkv_file.good()) {
      std::cerr << "Error: MKV file does not exist: " << opts.mkv_file_path << std::endl;
      exit(1);
    }

    // A C++ Wrapper for the k4a_playback_t does not exist, so we use the C API directly
    if (k4a_playback_open(opts.mkv_file_path.c_str(), &kinect_mkv_playback_handle) != K4A_RESULT_SUCCEEDED) {
      std::cout << "\033[1;31mFailed to open dummy file for playback\033[0m" << std::endl;
      exit(1);
    }
    ...
    if (k4a_playback_get_calibration(kinect_mkv_playback_handle, &kinect_calibration) != K4A_RESULT_SUCCEEDED) {
      std::cout << "\033[1;31mFailed to get calibration from dummy file\033[0m" << std::endl;
      exit(1);
    }
